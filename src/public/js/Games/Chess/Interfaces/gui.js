	    ...
	    if(GameBoard.side == COLOURS.WHITE) {
	      $("#GameStatus").text("GAME OVER {black mates}");
				if(GameBoard.side === GameController.PlayerSide)
					swal("GAME Over", "BLACK mates!", "success");
				else
					swal("GAME Over", "BLACK mates!", "danger");
				return true;
        } else {
	      $("#GameStatus").text("GAME OVER {white mates}");
				if(GameBoard.side === GameController.PlayerSide)
					swal("GAME Over", "WHITE mates!", "success");
				else
					swal("GAME Over", "WHITE mates!", "danger");
				return true;
        }
    }
...
GUI.ClickedSquare = function(pageX, pageY) {
	...
	console.log("clicked:" + GUI.ParseMove(sq));
...
GUI.PreSearch = function() {
		if(GameController.GameOver != true) {
			SearchController.thinking = true;
			$('#ThinkingImageDiv').append('<image src="/public/img/chesspieces/wikipedia/loading_spinner.gif" id="ThinkingPng"/>')
			setTimeout( function() {GUI.StartSearch(); }, 200);
		}
}

GUI.MakeUserMove = function() {
	if(UserMove.from != SQUARES.NO_SQ && UserMove.to != SQUARES.NO_SQ) {
		console.log("User Move:" + GUI.ParseMove(UserMove.from) + GUI.ParseMove(UserMove.to));
...
$(document).on('click','.Piece', function (e) {
	...
	if(SearchController.thinking == false && GameController.PlayerSide == GameBoard.side) {
...
$(document).on('click','.Square', function (e) {
	...
	if(SearchController.thinking == false && GameController.PlayerSide == GameBoard.side && UserMove.from != SQUARES.NO_SQ) {
		UserMove.to = GUI.ClickedSquare(e.pageX, e.pageY);
		GUI.MakeUserMove();
	}
});
...
GUI.RemoveGUIPiece = function(sq) {
	//console.log("remove on:" + GUI.ParseMove(sq));
...
GUI.MoveGUIPiece = function(move) {
	...
	if(move & MFLAGEP) {
		...
		console.log("en pas removing from " + GUI.ParseMove(epRemove));
    ...
    if(prom != PIECES.EMPTY) {
		console.log("prom removing from " + GUI.ParseMove(flippedTo));
    	GUI.RemoveGUIPiece(flippedTo);
...
GUI.StartSearch = function() {
	...
	// TODO Move.MakeMove here on internal board and GUI
	Move.MakeMove(SearchController.best);
	GUI.MoveGUIPiece(SearchController.best);
	$('#ThinkingPng').remove();
	GUI.CheckAndSet();
}

$("#TakeButton").click(function () {
	console.log('TakeBack request... GameBoard.hisPly:' + GameBoard.hisPly);
	if(GameBoard.hisPly > 1) {
		Move.TakeMove();
        Move.TakeMove();
		GameBoard.ply = 0;
...
function NewGame() {
	GameBoard.ParseFen(START_FEN);
	GameBoard.PrintBoard();
	GUI.SetInitialBoardPieces();
	GameController.PlayerSide = GameBoard.side;
	GUI.CheckAndSet();
