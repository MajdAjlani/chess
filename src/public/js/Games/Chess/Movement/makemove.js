...
Move.MovePiece = function(from, to) {
...
Move.MakeMove =function(move) {
    ...
    if ((move & MFLAGEP) != 0) {
        ...
        switch (to) {
            case SQUARES.C1:
                Move.MovePiece(SQUARES.A1, SQUARES.D1);
                break;
            case SQUARES.C8:
                Move.MovePiece(SQUARES.A8, SQUARES.D8);
                break;
            case SQUARES.G1:
                Move.MovePiece(SQUARES.H1, SQUARES.F1);
                break;
            case SQUARES.G8:
                Move.MovePiece(SQUARES.H8, SQUARES.F8);
                break;
            default:
                break;
        }
...
Move.TakeMove = function() {
    ...
    if ((MFLAGEP & move) != 0) {
        if (GameBoard.side == COLOURS.WHITE) {
            Move.AddPiece(to - 10, PIECES.bP);
        } else {
            Move.AddPiece(to + 10, PIECES.wP);
        }
    } else if ((MFLAGCA & move) != 0) {
        switch (to) {
            case SQUARES.C1:
                Move.MovePiece(SQUARES.D1, SQUARES.A1);
                break;
            case SQUARES.C8:
                Move.MovePiece(SQUARES.D8, SQUARES.A8);
                break;
            case SQUARES.G1:
                Move.MovePiece(SQUARES.F1, SQUARES.H1);
                break;
            case SQUARES.G8:
                Move.MovePiece(SQUARES.F8, SQUARES.H8);
                break;
            default:
                break;
        }
    ...
    var captured = CAPTURED(move);
    if (captured != PIECES.EMPTY) {
        Move.AddPiece(to, captured);
    }

    if (PROMOTED(move) != PIECES.EMPTY) {
        Move.ClearPiece(from);
        Move.AddPiece(from, (PieceCol[PROMOTED(move)] == COLOURS.WHITE ? PIECES.wP : PIECES.bP));
    }
}
